...
public class ReceiverTask implements Runnable {
	...
	private static Logger logger = Logger
			.getLogger("com.glavsoft.rfb.protocol.ReceiverTask");
	...
	int imageNumber = 0;
	// Variable para pausar la tarea de recibir
	private boolean pauseTask = false;

	public ReceiverTask(Reader reader, IRepaintController repaintController,
		...
		renderer = repaintController.createRenderer(reader, context
				.getFbWidth(), context.getFbHeight(), context.getPixelFormat());
		fullscreenFbUpdateIncrementalRequest = new FramebufferUpdateRequestMessage(
				0, 0, context.getFbWidth(), context.getFbHeight(), true);
	}
			...
			while (!pauseTask) {
				...
				try {
					...
					case SET_COLOR_MAP_ENTRIES:
						logger
								.severe("Server message SetColorMapEntries is not implemented. Skip.");
						setColorMapEntries();
						break;
					case BELL:
					...
					case SERVER_CUT_TEXT:
						...
						break;
					default:
						logger.severe("Unsupported server message. Id = "
								+ messageId);
					}
	...
	public void framebufferUpdateMessage() throws Exception {
...

		{
			...
			logger.finest(rect.toString()
					+ (0 == numberOfRectangles ? "\n---" : ""));
			if (decoder != null) {
				...
				synchronized (renderer) {
					renderer = repaintController.createRenderer(reader,
							rect.width, rect.height, context.getPixelFormat());
				}
				context.sendMessage(new FramebufferUpdateRequestMessage(0, 0,
						rect.width, rect.height, false));
				// repaintController.repaintCursor();
			} else
				throw new CommonException("Unprocessed encoding: "
						+ rect.toString());
		}
	...
	public synchronized void queueUpdatePixelFormat(PixelFormat pf) {
		...
		// context.sendMessage(new FramebufferUpdateRequestMessage(0, 0, 1, 1,
		// false));
	}
	...
	// ---------------
	  ...
	  public boolean isPauseTask() { return pauseTask; }
	 

	public void setPauseTask(boolean pauseTask) {
